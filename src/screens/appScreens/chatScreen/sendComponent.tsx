...
interface SendComponentProps {
  item: {
    ...
    message_type: string;
    timestamp: string;
    raw: {sender_details: {avatar: string[]; username: string}};
    deleted_for?: string;
    thumbnail?: string;
  };
...
const SendComponent: React.FC<SendComponentProps> = ({item, cb}) => {
  // if (item.deleted_for === global?.userData?.id) {
  //   return null;
  // }

  // const avatarUri = item.raw?.sender_details?.avatar?.[0]
  //   ? `${IMAGE_URL}${item.raw.sender_details.avatar[0]}`
  //   : undefined;
        ...
        {/* {avatarUri ? (
          ...
          <View style={styles.avatarPlaceholder}>
            <Text style={styles.avatarText}>
              {item.raw?.sender_details?.username
                ?.substring(0, 1)
                ?.toUpperCase() || 'U'}
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  timestamp: {
    ...
    color: Colors.primary.BLACK_FADE,
    marginTop: 4,
    textAlign: 'right',
  },
});
